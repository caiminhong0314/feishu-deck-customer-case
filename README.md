...
该技能基于杰森的 `feishu-deck-h5` 衍生，需先安装并配合基础 Deck 技能使用。在部分本地环境中，该基础技能可能显示为 `Feishu Deck 0613`。
...
1. 需先安装 `feishu-deck-h5` 或等价的飞书 Deck 生成技能。
2. 需要具备文档阅读权限和图片下载权限。
3. AI 第一版图片选择不一定完全准确，需结合对客讲解逻辑确认关键素材。
4. 案例制作仍需要制作人对业务场景、故事主线和证明重点有清晰判断，不能完全依赖 AI 做重点选择和内容展示。
